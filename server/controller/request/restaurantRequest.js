// controller/request/restaurantRequest.js
...
export function buildCreatePayload(req) {
    ...
    for (const k of ['kakaoPlaceId', 'regionCode','foodTagCode',
        'mainFood', 'phone', 'address']) {
...
export function buildUpdatePayload(req) {
    ...
    for (const k of ['longitude','latitude'])
        if (k in body) payload[k] = parseNumber(body[k], k, { nullable: true });
