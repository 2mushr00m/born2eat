...
function getParserVersion(episode_no) { // parser 버전 결정
    if (episode_no <= 468) return 1;
    return 2;
}
function getEpisode($) {    // 에피소드 번호 추출
    ...
    if ($ep.length === 0)
        throw new BasicError('TASTY_GUYS 회차 정보를 찾을 수 없습니다.', {
            code: 'EPISODE_NOT_FOUND', status: 502, location: loc(getEpisode)
        });
    ...
    const ep = parseNumber(match?.[1], 'episode', {
        location: loc(getEpisode), integer: true, positive: true, code: 'EPISODE_NOT_FOUND', status: 502,
    });
...
const TASTY_GUYS = {
    ...
    async parseEpisode(episode_no) { 
        ...
        /** @type {Result} */
        const ret = {
            parser_ver, episode_no, crawl_url,
            status: null, message: null,
            restaurants: [], aired_at: null,
        };
