...
export async function updateUser(userId, payload, opt) {
    ...
    const ALLOW = isAdmin
        ? { email: true, nickname: true, phone: true, profileUrl: true, status: true, suspendedUntil: true }
    ...
    try {
        ...
        if (e?.code === 'ER_DUP_ENTRY') {
            if ('nickname' in patch) throw new AppError(ERR.CONFLICT, {
                message: '이미 사용 중인 닉네임입니다.', data: { keys: ['nickname'], dbCode: err?.code } });
            if ('email' in patch) throw new AppError(ERR.CONFLICT, {
                message: '이미 사용 중인 이메일입니다.', data: { keys: ['email'], dbCode: err?.code } });
            throw new AppError(ERR.CONFLICT, {
                message: '중복된 값이라 수정할 수 없습니다.', data: { dbCode: err?.code } });
        }
        if (err instanceof AppError) throw err;
        throw new AppError(ERR.DB, {
            message: '회원정보 수정 중 오류가 발생했습니다.',
            data: { targetId: userId, keys: Object.keys(patch), dbCode: err?.code, extra: { mode, patch } },
            cause: err,
        });
    } finally {
        conn.release();
...
export async function deleteUser(userId) {
    ...
    try {
        ...
        // 동시 요청 경합 방지
        const [rows] = await conn.execute(
            `SELECT user_id, status FROM user WHERE user_id = :userId FOR UPDATE`,
        { userId });

        const user = rows?.[0];
        if (!user) throw new AppError(ERR.NOT_FOUND, {
            message: '해당 계정이 존재하지 않습니다.',
            data: { targetId: userId } });
