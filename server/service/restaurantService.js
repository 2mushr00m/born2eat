...
export async function readRestaurantList(filter, opt = {}) {
    ...
    try {
        ...
        const mainPhotoById = new Map(photoRows.map((r) =>
            [r.restaurantId, r.filePath]));
        for (const it of items)
            it.mainPhoto = mainPhotoById.get(it.restaurantId) ?? null;
        ...
        for (const it of items)
            it.tags = tagsById.get(it.restaurantId) ?? [];
    ...
    try {
        ...
        for (const p of photoRows ?? []) {
            const photo = {
                ...
                sourceUserNickname: p.sourceType === RESTAURANT_PHOTO_SOURCE.USER
                    ? (p.sourceUserNickname ?? null) : null,
            };

            if (p.photoType === RESTAURANT_PHOTO_TYPE.MAIN)
                detail.photos.main.push(photo);
            else if (p.photoType === RESTAURANT_PHOTO_TYPE.MENU_BOARD)
                detail.photos.menuBoard.push(photo);
        ...
        if (brRows?.length) {
            ...
            for (const bid of broadcastIds)
                ottByBid.set(bid, { NETFLIX: null, TVING: null, WAVVE: null, etc: null });
...
export async function deleteRestaurant(restaurantId) {
    ...
    try {
        ...
        const [result] = await conn.execute(
            `DELETE FROM restaurant WHERE restaurant_id = :restaurantId`, { restaurantId });
