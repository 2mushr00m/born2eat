 ...
 */
export default async function runCrawl({ broadcast_code, episode_from, episode_to, delayMs = 1500 }) {
  ...
  // 2) 종료 회차 결정
  let toEp = episode_to;
  if (toEp == null) toEp = await scraper.latestEpisode();
    ...
    for (let ep = episode_from; ep <= toEp; ep += 1) {
      ...
      try {
        ...
        // 3-4) 비정상 경로
          ...
          throw new BasicError('알 수 없는 크롤 상태입니다.', {
