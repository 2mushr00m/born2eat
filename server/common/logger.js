...
function normalize(payload) {
  ...
  if (data && typeof data === 'object' && !Array.isArray(data)) {
    ...
    const onlyMethodPath = keys.length > 0 && keys.every((k) => k === 'method' || k === 'path');
...
function formatDebug({ code, message, data }) {
  ...
  if (data !== undefined && data !== null) lines.push(`${COLORS.debugData}${formatDataBlock(data)}${COLORS.reset}`);
